...
**Status:** first-pass draft. Subsequent revisions will polish prose, add external citations, refine figure captions, and adjudicate the framing decisions surfaced in §9 and §10.
...
The Tier 1.C cross-sweep audit identified `noise_sweep` `σ = 0.2` as a second §4.2 instance with empirical evidence at least as strong as the alignment instance, and on one component (mean-peak) considerably stronger. The bimodality signature is the strongest in the entire 11-sweep audit: dip p = 7.6×10⁻⁶ (four orders of magnitude beyond the 0.20 criterion threshold), KDE modes = 2, std/IQR = 0.544, n = 310 windows pooled across 10 seeds. The two modes are unambiguous in the per-window histogram. Cross-seed mean Φ = 144.63 (95% CI [125.58, 163.49]).
...
1. The bimodality dip test result (p = 7.6×10⁻⁶) is four orders of magnitude stronger than at alignment w_a = 0.6 (p = 0.102).
...
### 5.5 §4.3 verdict — jamming-specific scope

The §4.3 compressibility-Φ-inversion is **confirmed at jamming α = 0.2 and is sweep-specific within the existing sweep set**. The cross-sweep audit's §4.3 sub-table (Tier 1.C) tested every other low-phi_norm-σ candidate against the four §4.3 descriptors (σ_u, phi_norm σ, dip p, Φ-vs-baseline direction). Only `jamming α = 0.5` is partially present (σ_u = 0.5331, phi_norm σ = 0.0441, +4% Φ vs α = 1.0 — an intermediate compressibility regime, not a confirmed instance). All other candidates fail at least one load-bearing descriptor:

- `alignment w_a = 0.0`: σ_u = 0.7941 (in the disordered-random regime, not floor-locked moderate); the structure is "no alignment, cohesion+separation produce structure," not compressibility-prevention.
- `leader λ = 1.6` and `λ = 2.4`: Φ inverted *opposite* direction (collapse below baseline, not above) — the Outcome 4 leader-block partition mechanism.
- `noise σ = 0.5`: Φ drops at the random regime (B&S random endpoint) — not the §4.3 inversion direction.

The original Phase 5 pre-registration anticipated a multi-sweep §4.3 generalization (jamming + leadership + possibly alignment via the transitional-peak interpretation). The empirical verdict narrows this: §4.3 is a **jamming-specific finding** within the existing sweep set, with the leadership extension explicitly tested and disconfirmed. This is methodologically honest scope-narrowing — the framework's empirical reach is jamming for §4.3, not jamming + leadership. The Phase 5 result is stronger, not weaker, for shipping a tested-and-disconfirmed extension claim alongside the confirmed primary claim: ambiguity in either direction would have been a less defensible posture.
...
At λ = 2.4: MI mean is **low** (0.19, closer to coherent's 0.10 than to compressibility's 0.49); spatial dependence ρ(MI, −distance) is **moderate-strong** (+0.30, not compressibility's near-zero ≲ 0.15); the eigenvalue gap is **intermediate** (7.4, well below compressibility's rank-1-dominated 18.7); and Fiedler ⟷ kmeans-2 agreement is at chance (0.49).
...
**Finer noise-sweep grid around σ = 0.2** (`{0.15, 0.2, 0.25, 0.3, 0.35}`) for the analogous question on the noise axis. The current grid jumps from σ = 0.1 (modes = 3, dip p = 0.45) to σ = 0.2 (bimodal, dip p = 7.6×10⁻⁶) to σ = 0.5 (random unimodal). The dip p = 7.6×10⁻⁶ at σ = 0.2 is the strongest bimodal signature in the audit and is worth characterizing precisely.
...
- `outputs/comparison/cross_sweep/scenario_agreement_spearman.csv` (commit `f0ad4aa`) — 38 × 18 Spearman agreement matrix; load-bearing for §7.
...
*End of Phase 5 internal report draft. Subsequent revisions will polish prose, refine figure captions, add external citations beyond the methodology references, and adjudicate the framing decisions surfaced in §9 and §10. The four prose-engagement items from the figure review are addressed in §4.3 (asymmetric §4.2 evidence), §6.3 (standalone MI matrix diagnostic), §8.4 (noise σ = 0.5 reclassification), and §7.2 (figure 4 hierarchical-clustering separation of §4.2 instances) — explicitly engaged rather than flattened into smooth narrative.*
